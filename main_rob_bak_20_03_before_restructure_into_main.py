...
def make_datasets(cl_dset, datapath, randomise=False, show_dset_img_sample=False, premade_tasks=None):
    ...
    for task_no in range(cl_dset['meta']['task_no']):
        ...
        if show_dset_img_sample:
            ...
            print("\n\n")
    return datasets, all_classes


datasets, _ = make_datasets(cl_dset, datapath, randomise=False)
premade_tasks = [["n02958343", "n02109961", "n02100735"], ["n03179701", "n04209133", "n02100583"], ["n03599486","n02802426", "n02101388"], ["n04330267", "n02106166", "n02099601"], ["n02100877", "n04118538", "n02107142"], ["n02099267", "n02106662", "n02101006"], ["n02108551", "n02108089", "n02109047"]]
shuffled_datasets, shuffled_tasks = make_datasets(cl_dset, datapath, premade_tasks = premade_tasks, randomise=True)
...
def train_datasets(trainer, datasets, ewc=False, spec_decoup = False, save_appendix="", start_from_task=0, save_at_least_end_task=True):
...
if True:
    ...
    sec_split_file_name = "cl_t5_c8"
    sec_cl_dset = get_cl_dset(os.path.join(datapath, sec_split_file_name + ".txt"))
    sec_datasets, _ = make_datasets(sec_cl_dset, datapath, randomise=False)
    sec_shuffled_datasets, sec_shuffled_tasks = make_datasets(sec_cl_dset, datapath, randomise=True)
    import json
    with open(f'latest_shuffled_tasks_{sec_split_file_name}.txt', 'w') as filehandle:
        json.dump(sec_shuffled_tasks, filehandle)

    trainer.set_model(model_type, cl_dset)
    train_datasets(trainer, datasets, spec_decoup=True, ewc=False,
                   save_appendix=f"_{dataset_name}_{split_file_name}")

    # trainer.set_model(model_type, cl_dset)
    # train_datasets(trainer, shuffled_datasets, spec_decoup = True, ewc=False,
    #                                   save_appendix=f"_{dataset_name}_{split_file_name}_shuffled")

    trainer.set_model(model_type, sec_cl_dset)
    train_datasets(trainer, sec_datasets, spec_decoup=True, ewc = False, save_appendix=f"_{dataset_name}_{sec_split_file_name}",
                             start_from_task=0)

    trainer.set_model(model_type, sec_cl_dset)
    train_datasets(trainer, sec_shuffled_datasets, spec_decoup=True, ewc = False, save_appendix=f"_{dataset_name}_{sec_split_file_name}_shuffled",
                   start_from_task=0)

    trainer.set_model(model_type, cl_dset)
    train_datasets(trainer, datasets, spec_decoup = True, ewc = True,
                   save_appendix=f"_{dataset_name}_{split_file_name}_shuffled")

    trainer.set_model(model_type, cl_dset)
    train_datasets(trainer, shuffled_datasets, spec_decoup = True, ewc = True,
                   save_appendix=f"_{dataset_name}_{split_file_name}_shuffled")

    trainer.set_model(model_type, sec_cl_dset)
    train_datasets(trainer, sec_datasets, spec_decoup = True, ewc = True,
                   save_appendix=f"_{dataset_name}_{sec_split_file_name}",
                   start_from_task=0)

    trainer.set_model(model_type, sec_cl_dset)
    train_datasets(trainer, sec_shuffled_datasets, spec_decoup = True, ewc = True,
                   save_appendix=f"_{dataset_name}_{sec_split_file_name}_shuffled",
                   start_from_task=0)