...
def make_datasets(cl_dset, datapath, randomise = False, show_dset_img_sample = False, premade_tasks = None):
    ...
    for task_no in range(cl_dset['meta']['task_no']):
        task = get_task(cl_dset, task_no, datapath, verbose=True) if premade_tasks is None else premade_tasks[task_no]  # list of classes (strings) corresponding to task
...
if False:
    ...
    for task in range(cl_dset['meta']['task_no']):
        for model_type in model_types:
            _, test_dsload_task = torch.utils.data.random_split(datasets[task], [int(len(datasets[task]) * 0.8), len(datasets[task]) - int(len(datasets[task]) * 0.8)])
            test_loaders.append(torch.utils.data.DataLoader(test_dsload_task, batch_size=trainer.batch_size, shuffle=True, drop_last=False))
            _, shuff_test_dsload_task = torch.utils.data.random_split(fixed_shuffled_dsets[task],[int(len(fixed_shuffled_dsets[task]) * 0.8), len(fixed_shuffled_dsets[task]) - int(len(fixed_shuffled_dsets[task]) * 0.8)])
            shuff_test_loaders.append(torch.utils.data.DataLoader(shuff_test_dsload_task, batch_size=trainer.batch_size, shuffle=True, drop_last=False))
...
def train_datasets(trainer, datasets, ewc = False, save_appendix = "", start_from_task = 0, save_at_least_end_task = True):
    ...
    for task_no in range(len(datasets)):
        ...
        if task_no >= start_from_task:
            metrics_task, fisher_diag_task = trainer.train(train_loaders, test_loaders, ewc = ewc, prev_fisher = fisher_diag, prev_params = prev_params)
            ...
            if ewc:
                ...
                if fisher_diag is None:
                    ...
                    assert set(fisher_diag.keys()) == set(fisher_diag_task.keys()), f"The previous fisher diagonal and the one obtained from {task_no} are dicts not corresponding to same parameters!"
...
if True:
    trainer.save()
    metrics = train_datasets(trainer, datasets, ewc = False, save_appendix = f"_{dataset_name}_{split_file_name}", start_from_task = 0)
    ...
    metrics_shuffled = train_datasets(trainer, shuffled_datasets, ewc = False, save_appendix = f"_{dataset_name}_{split_file_name}_shuffled")

    trainer.set_model(model_type, cl_dset)
    metrics_ewc = train_datasets(trainer, datasets, ewc = True, save_appendix = f"_{dataset_name}_{split_file_name}", start_from_task = 0)
    
    trainer.set_model(model_type, cl_dset)
    metrics_shuffled_ewc = train_datasets(trainer, shuffled_datasets, ewc = True, save_appendix = f"_{dataset_name}_{split_file_name}_shuffled")
